<script lang="ts">
  ...
  function drawLRHeatmap() {
    ...
    g.selectAll<SVGTextElement, LRDatum>('text.cell-val')
      .data(lrdata.filter(d => d.count > 0)).join('text')
      .attr('class', 'cell-val')
    ...
    // axis titles
    svg.append('text')
      .attr('x', gx + gridW / 2).attr('y', totalH - 2)
      .attr('text-anchor', 'middle')
    ...
    cts.forEach(ct => {
      ...
      topG.append('rect')
        .attr('x', x(ct)!).attr('y', BAR_SIZE - 4 - h)
  ...
  function drawTFHeatmap() {
    ...
    const allVals = [
      ...cts.map(ct => tfldata[ct] ?? 0),
      ...cts.map(ct => rtfdata[ct] ?? 0),
    ];
    const heatColor = d3.scaleSequential()
